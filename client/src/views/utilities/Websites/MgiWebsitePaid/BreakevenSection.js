  ...
  return (
    <Box sx={{ m: 3 }}>
      ...
      <Grid container spacing={3}>
        {candles
          .filter((candle) => candle.breakeven_candle) // Only include candles that have an image
          ...
          <Grid item xs={12} sm={6} md={4} key={candle.id}>
            <Card>
              <CardMedia
                component="img"
                height="200"
                image={candle.breakeven_candle}
                alt="Breakeven Candle"
                onClick={() => handleOpenImageModal(candle.breakeven_candle)}
                sx={{ cursor: 'pointer' }}
              />
              <CardContent>
                {/* Trade Signal */}
